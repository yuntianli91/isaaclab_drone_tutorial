...
parser = argparse.ArgumentParser(
  description="Create parallel Crazyflie scenes with static pillars."
)
parser.add_argument(
...
parser.add_argument(
  ...
  help="Distance in metres between neighbouring environment origins.",
)
parser.add_argument(
  "--max_steps", type=int, default=500,
  help=(
    "Number of physics steps before exit; use 0 to run until the app "
    "closes."
  ),
)
parser.add_argument(
  "--reset_interval", type=int, default=200,
  help=(
    "Physics steps between UAV state resets; use 0 to disable periodic "
    "resets."
  ),
)
...
def reset_uavs(scene: InteractiveScene) -> None:
  ...
  uav.write_joint_state_to_sim(
    uav.data.default_joint_pos, uav.data.default_joint_vel
  )
  scene.reset()
...
def print_scene_state(scene: InteractiveScene) -> None:
  ...
  print(
    f"[INFO] first {preview_count} environment origins:\n"
    f"{scene.env_origins[:preview_count]}"
  )
  print(
    f"[INFO] first {preview_count} UAV positions relative to origins:\n"
...
def main() -> None:
  ...
  print(
    f"[INFO] Scene ready: num_envs={scene.cfg.num_envs}, "
