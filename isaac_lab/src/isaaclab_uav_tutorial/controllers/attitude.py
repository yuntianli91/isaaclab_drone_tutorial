...
class AttitudeControllerCfg:
  ...
  attitude_integral_limit: float           # 姿态误差积分限幅 (s)。
  moment_scale: float                      # 三轴最大合力矩 (N·m)。
...
class AttitudeController:
  ...
  def __init__(self, num_envs: int, device: str, control_dt: float,
               cfg: AttitudeControllerCfg) -> None:
    """初始化增益和每个环境独立的姿态积分状态。
    ...
    self.attitude_kp = torch.tensor(self.cfg.attitude_kp, device=self.device)
    self.attitude_ki = torch.tensor(self.cfg.attitude_ki, device=self.device)
    self.attitude_kd = torch.tensor(self.cfg.attitude_kd, device=self.device)
    self.attitude_error_integral = torch.zeros(num_envs, 3,
                                               device=self.device)
